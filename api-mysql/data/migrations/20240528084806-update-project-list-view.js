...
const { VIEWS: { PROJECT_LIST_VIEW } } = require('../../src/utils/constants');

/** @type {import('sequelize-cli').Migration} */
module.exports = {
  async up(queryInterface, Sequelize) {
    await queryInterface.sequelize.query(`DROP VIEW IF EXISTS ${PROJECT_LIST_VIEW}`)

    await queryInterface.sequelize.query(`CREATE VIEW ${PROJECT_LIST_VIEW} AS
    SELECT
        id AS project_id,
        ...
        (SELECT CAST(SUM(COALESCE(factor.scope1, 0) + COALESCE(factor.scope2, 0) + COALESCE(factor.scope3, 0)) as decimal(44,5)) 
            FROM project
              ...
              LEFT JOIN emission ON resource.emission_id = emission.id
              LEFT JOIN emissions_factor factor ON
              emission.id = factor.emission_id) 
            AS totalReduction,
